...
class MQTTConnection {
    ...
    sendUdpMessage(payload, timestamp) {
        if (!this.udp.remoteAddress) {
            console.warn(`[audio-diag] device=${this.clientId} dropped=${payload.length} reason=no-udp-remote-address`);
    ...
    onUdpMessage(rinfo, message, payloadLength, timestamp, sequence) {
        ...
        if (this.udp.remoteAddress !== rinfo) {
            ...
            console.warn(`[audio-diag] device=${this.clientId} udp-remote=${rinfo.address}:${rinfo.port}`);
        }
...
class MQTTServer {
    constructor() {
        ...
        this.apiPort = parseInt(process.env.API_PORT) || 18007;
    ...
    start() {
        ...
        this.apiServer.listen(this.apiPort, '0.0.0.0', () => {
            console.warn(`主动提醒 API 正在监听端口 ${this.apiPort}`);
        });
    ...
    findDevice(deviceId) {
        const normalized = String(deviceId || '').trim().toLowerCase().replace(/_/g, ':');
        return Array.from(this.connections.values()).find(
            connection => String(connection.macAddress || '').toLowerCase() === normalized
        );
    }
    ...
    handleApiRequest(request, response) {
        if (request.method === 'GET' && request.url === '/health') {
            this.writeJson(response, 200, {
                ok: true,
                connectedDevices: Array.from(this.connections.values()).map(
                    connection => connection.macAddress
                )
            });
            return;
        }

        if (request.method !== 'POST' || request.url !== '/study-alert') {
        ...
        request.on('end', () => {
            try {
                ...
                const device = this.findDevice(payload.device_id);
                if (!text) {
                    this.writeJson(response, 400, { ok: false, error: 'text_required' });
                    return;
                }
                if (!device) {
                    this.writeJson(response, 409, { ok: false, error: 'device_offline' });
                    return;
                }

                const requestId = String(payload.request_id || crypto.randomUUID());
                device.sendMqttMessage(JSON.stringify({
                    type: 'study_alert',
                    request_id: requestId,
                    text,
                    status: String(payload.status || '学习提醒'),
                    emotion: String(payload.emotion || 'neutral'),
                    interrupt: payload.interrupt !== false
                }));
                console.warn(`主动提醒已下发: ${device.macAddress} ${requestId}`);
